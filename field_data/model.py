...
from .config import BASE_CH, AX_HEADS, USE_AXIAL
...
class AxialMHSA(nn.Module):
    def __init__(self, ch, heads=AX_HEADS):
        super().__init__()
        self.mha = nn.MultiheadAttention(ch, heads, batch_first=True)

    def forward(self, x):  # [B,C,H,W] attention along W (trace axis)
        B, C, H, W = x.shape
...
class ResUNet(nn.Module):
    def __init__(self, in_ch=3, base_ch=BASE_CH, time_dim=64, time_emb_dim=256,
                 use_axial=USE_AXIAL, ax_heads=AX_HEADS):
        super().__init__()
        ...
        self.ax2 = AxialMHSA(base_ch * 2, heads=ax_heads) if use_axial else nn.Identity()
        ...
        self.ax3 = AxialMHSA(base_ch * 4, heads=ax_heads) if use_axial else nn.Identity()
